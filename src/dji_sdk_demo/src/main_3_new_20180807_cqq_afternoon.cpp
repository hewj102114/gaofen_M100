...
float serach_flight_height = 3.0;   //2018-08-05
...
int count_num=0;
int Pad_distance_list_all = 0; //2018-08-06 
	...
	switch (state_in_mission) 
	{
	  case 0://take off
	  {
		...
		writeF<<"begin take off start yaw = "<<start_yaw<<endl;
	      ...
	      {
                if ( ob_distance[0]<serach_flight_height-0.1) //adjust height
		{
		    flying_height_control_tracking += 0.003;
		}
		else if ( (ob_distance[0]>serach_flight_height+0.1)&&ob_distance[0]<5)
		{ 
		    flying_height_control_tracking -= 0.003;
		}
...
/********************************************************************************************************************************/	  
	  ...
	  {
	      Pad_distance_list_all = Pad_distance_list2; 
...
/********************************************************************************************************************************/	  
	  ...
	  {
	      Pad_distance_list_all = Pad_distance_list3; 
...
/********************************************************************************************************************************/	  
	  ...
	  {
	      Pad_distance_list_all = Pad_distance_list4; 
...
/********************************************************************************************************************************/	  
	  ...
	  {
	      Pad_distance_list_all = Pad_distance_list5; 
...
/********************************************************************************************************************************/	  
	  case 9: //for parking pad 9
	  {
	      ...
	      Pad_distance_list_all = Pad_distance_list6; 
...
//     {
// 	count_num++;
// 	drone->attitude_control(0x4B,0.7,0,0,0); // go forward
// 	if(count_num > Pad_distance_list_all) //for middle searching
// 	{ 
// 	  count_num=0;
// 	  land_mode=1;
// 	  drone->attitude_control(0x4B,0,0,0,0);
...
//     }
    ...
    if(land_mode==0) //again for the same landing height
    {
        ...
        else if ( (ob_distance[0]>height+0.1)&&ob_distance[0]<6)
        { 
        flying_height_control_tracking -= 0.003;
        }
    ...
    if(land_mode==1&&drone->gimbal.pitch<-85)   //closing to landing part and landing
    {
        ...
        {
// 	  if(abs(g_pos_y) < 1.5)
// 	  drone->attitude_control(0x4B,0,0.4*fly_Y_direction,0,0); 
// 	  else
// 	  drone->attitude_control(0x4B,0,0,0,0); 
      }
    } 
	...
	if ((ob_distance[0] > serach_flight_height)&& (ob_distance[0] < 4))
	{
	    drone->attitude_control(0x4B,0,0,0,0);
	    take_off_flag  = 1;
	    land_mode = 0;
