...
//		   if (Th1 > 1.4 && Th2 > 1.4 && Th4 > 50 && Th5 >60&&abs(Th3)<50)       // 2018-07-31-18:55  beside the building
		...
		//	  if (Th1 > 1.3 && Th2 > 1.3  && Th4 > 150 && Th5 > 50&&abs(Th3)<50)
					...
					else
					 {
								 ...
								 if (Th5 > 120 && Th6 > 100 &&abs(Th3)<50)  //pad 			
									{  
										RedChannel.at<uchar>(j, i) = 255;
										GreenChannel.at<uchar>(j, i) = 255;
										BlueChannel.at<uchar>(j, i) = 255;
...
//					      printf("test...........\n");
						...
						//计算轮廓的面积  
		//				cout << "【筛选前总共轮廓个数为】：" << (int)contours_out.size() << endl;
		...
		//				cout << "【筛选后总共轮廓个数为：" << (int)contours_out.size() << endl;
						for (int i = 0; i < (int)contours_out.size(); i++)
						{
						double g_dConArea = contourArea(contours_out[i], true);
		//				cout << "【用轮廓面积计算函数计算出来的第" << i << "个轮廓的面积为：】" << g_dConArea << endl;
...
*/
						...
						writeF11<<"detection_flag="<<detection_flag.data<<endl;   // write in log
