...
ofstream writeF ( "/home/ubuntu/GaofenChallenge/log100.txt");
...
{	
        resize(mimg, mimg, cv::Size(640,360), 0, 0, INTER_LINEAR); //Moban need to be resized
	      vector<Mat> channels;
        ...
        for (int i = 0; i < img.cols; i++)
        {
            ...
            //	if (Th1 > 2.5 && Th2 > 2.5  && Th4 > 80 && Th5 > 70&&abs(Th3)<50)
            //	if (Th1 > 1.1&& Th2 > 1.1  && Th4 > 50 && Th5 > 40&&abs(Th3)<50)
            //	if (Th1 > 1.2 && Th2 > 1.2  && Th4 > 50 && Th5 > 45&&abs(Th3)<50)
            //    if (Th1 > 1.7 && Th2 > 1.7  && Th4 > 60 && Th5 > 60&&abs(Th3)<50)
                if (Th1 > 1.3 && Th2 > 1.3  && Th4 > 50 && Th5 > 50&&abs(Th3)<50)
                {
        ...
        medianBlur(img, img, 3);// useful
        threshold(img, img, 150, 255, 0);
        threshold(mimg, mimg, 150, 255, 0);
/*
       namedWindow( "mimg_thre", WINDOW_AUTOSIZE  ); // Create a window for display.
       imshow( "mimg_thre",mimg);
       namedWindow( "img_thre", WINDOW_AUTOSIZE  ); // Create a window for display.
       imshow("img_thre",img);
       //         waitKey(0);
*/
        vector< vector<Point> > contours, contours_out, contours_num, contours_test, contours_state, contours_result;

	      vector< vector<Point> > contours_out2;

        findContours(img, contours, CV_RETR_LIST, CV_CHAIN_APPROX_NONE);//rect + num + other
        findContours(img, contours_out, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);//rect
	      findContours(img, contours_out2, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);//rect
        findContours(mimg, contours_state, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);//moban num
        ...
        Mat num_result(img.size(), CV_8U, Scalar(0));//contours
        drawContours(num_result, contours, -1, Scalar(255), 2);
...
float point_y;
  ...
  //cout << "All numbers of contours: " << (int)contours_out2.size() << endl;
	for (int i = 0; i < (int)contours_out2.size(); i++)
	{
	  double g_dConArea = contourArea(contours_out2[i], true);
   //cout << "Using contourArea to compute the " << i << " area of contour is: " << g_dConArea << endl;
	}

	//delete samll areas
	vector <vector<Point>>::iterator itc_rect = contours_out2.begin();

	for (; itc_rect != contours_out2.end();)
	{
	double g_dConArea = contourArea(*itc_rect);
	if (g_dConArea < 7000) //2.8m rect = 10000
	{
	itc_rect = contours_out2.erase(itc_rect);
	}
	else
	{
	++itc_rect;
...
//	cout << "After delete numbers of contours: " << (int)contours_out2.size() << endl;
	for (int i = 0; i < (int)contours_out2.size(); i++)
	{
	  double g_dConArea = contourArea(contours_out2[i], true);
//	cout << "Using contourArea to compute the " << i << " area of contour is: " << g_dConArea << endl;
        ...
        if (contours_out2.size() == 1) //maybe bigger than 1,need to be filtered
        {
            vector<Point> pts1;
            approxPolyDP(Mat(contours_out2[0]), pts1, arcLength(Mat(contours_out[0]), true)*0.03, true);
//          printf("Size is %d\n",(int)pts1.size());
            ...
            {
                if(pts1[i].x+pts1[i].y<addmin)
                {
                    addmin=pts1[i].x+pts1[i].y;
                    ptfour[0]=pts1[i];
                }
                if(pts1[i].x+pts1[i].y>addmax)
                {
                    addmax=pts1[i].x+pts1[i].y;
                    ptfour[2]=pts1[i];
                }
                if(pts1[i].x-pts1[i].y<submin)
                {
                    submin=pts1[i].x-pts1[i].y;
                    ptfour[3]=pts1[i];
                }
                if(pts1[i].x-pts1[i].y>submax)
                {
                    submax=pts1[i].x-pts1[i].y;
                    ptfour[1]=pts1[i];
                }
            }
...
point_y = ptfour[0].y;
        ...
        float a = ptfour[1].x-ptfour[0].x+10;
				float b = ptfour[3].y-ptfour[0].y+10;
        Rect rect(ptfour[0].x,ptfour[0].y, a ,b);

				 rate1 = a/120;
				 rate2 = b/120;

	    if((ptfour[1].x+10)<640 &&(ptfour[3].y+10)<360) // avoid array getting out
	    {
		  Mat test  = num_result(rect);
		  Mat normal_img;
		  
		  resize(test, normal_img, cv::Size(120,120), 0, 0, INTER_LINEAR);

		  findContours(normal_img, contours_num, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
		  
		  vector< std::vector<Point> >::const_iterator itc3 = contours_num.begin();
		  
		  double minconform=0.5,conform;//minconform is helpful
		  
		  while (itc3 != contours_num.end())
		  {
		      conform = matchShapes(contours_state[0], *itc3, CV_CONTOURS_MATCH_I3, 0.0);
		      printf("conform is %f\n",conform);
		      writeF << "conform  =" << conform << endl;
		  ...
		  //  namedWindow( "normal_img", WINDOW_AUTOSIZE  ); // Create a window for display.
      //      imshow("normal_img",normal_img);
...
*/	
	...
	if (!contours_test.empty())  
	{
		contours_result.push_back(*(--itc4)); //contours of nums

		drawContours(result_num, contours_result, -1, Scalar(255), 2);

		Moments mom = moments(contours_result[0]);

		circle(result_num, Point(mom.m10 / mom.m00, mom.m01 / mom.m00), 10, Scalar(255), 2);
		Senter = Point(mom.m10 / mom.m00, mom.m01 / mom.m00);
//imshow("norm_center",result_num);
////////////?????????///////////////////////////////////////////////////////
//		ptfour[0].x,ptfour[0].y;
//				float rate1 = 120/a;
//				float rate2 = 120/b;
                   ...
                   char str[200];
                   sprintf(str,"%d,%d,%f,%f,%f,%f",Senter.x,Senter.y,rate1,rate2,point_x,point_y);
                   cout<<str<<endl;
            //putText(img,str,Point(30,30),CV_FONT_HERSHEY_SIMPLEX,1,Scalar(255),2);

		float c_x = Senter.x*rate1 + point_x;
		float c_y = Senter.y*rate2 + point_y;
                 cout<<c_x<<"  "<<c_y<<endl;
circle(img, Point(c_x,c_y), 10, Scalar(255,255,255), 5);
//imshow("img",img); 
waitKey(1);
...
pos_result.x = -(c_y-205)*0.04/5;  //5 pixles = 4cm  
pos_result.y = (c_x-320)*0.04/5;  //5 pixles = 4cm  (320,180)
		...
		{
		      vector<std::vector<Point>>::iterator itc = contours_out.begin(); //rect
		      Point2f senter;
			...
			if (INorOUT <= 0)
		      ...
		      drawContours(result_out, contours_out, -1, Scalar(255), 2); //rect
